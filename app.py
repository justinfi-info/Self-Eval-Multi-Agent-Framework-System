...
def planner_agent(state):
    ...
    Return only a stort actionable plan for the worker agent to follow.
    """
...
def worker_agent(state):
    ...
    write the best possible improved response. If the reviewer feeback exist, explain fix the issues mentioned in the feedback and improve the answer.
    """
    ...
    logger.info(f"Worker output: {state['worker_calls']} - {draft_response}")
    return state
...
def reviewer_agent(state):
    ...
    reason =reason_line.replace("Reason:", "").strip() if reason_line else "No Resaon Provided"
    ...
    logger.info(f"Reviewer output #{state['reviewer_calls']}: {decision} - {reason}")
    return state

def review_router(state):
    if state.get('review_decision') == "approve" or state.get('review_count')>=2:
        return "__end__"
    state['review_count'] = state.get('review_count', 0) + 1
    return "worker_agent"

# Worker Flow Agent Graph
...
try:
    ...
    with open("graph.png", "wb") as f:
        f.write(png_data)
    logger.info("Graph png saved as grapgh.png")
